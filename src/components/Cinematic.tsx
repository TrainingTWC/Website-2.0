...
export function CinematicHero({
  ...
  const copyParallaxY = useTransform(scrollYProgress, [0, 1], [0, -80]);
...
export function ChapterReveal({
  ...
  const copyOpacity = useTransform(
    source,
    [lerp(ls, le, 0.0), lerp(ls, le, 0.08), lerp(ls, le, 0.92), lerp(ls, le, 1.0)],
    [0, 1, 1, 0],
  );
            ...
            {(() => {
              ...
              return (
                <button
                  ...
                  className="relative block w-full max-w-[200px] sm:max-w-xs lg:max-w-md mx-auto aspect-[4/5] rounded-[2rem] lg:rounded-[2.5rem] overflow-hidden bg-natural-muted shadow-2xl group"
                >
                  <img
                    src={resolvedImage}
...
function ChapterCard({
  ...
  //
  //   â”Œ-- ENTER --â”¬--------- HOLD (parallax) ---------â”¬-- EXIT --â”
  //   start                                                       end
  //   0%         15%                                  85%       100%
  //
  ...
  //   * PRODUCT slides in from the RIGHT (translateX +60%, rotateY âˆ’24Â°,
  //     translateZ âˆ’500px) -> flies past camera to centre -> slides out
