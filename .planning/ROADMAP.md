...
Plans:
- [x] 02-01-PLAN.md — CartProvider + DiscountProvider + ToastProvider + CartPanelProvider (Wave 1)
- [x] 02-02-PLAN.md — SSR-unsafe dynamic() wraps + SmoothScroll use client (Wave 2, depends on 02-01)
...
*Last updated: 2026-05-26 — Phase 5 (API Security Proxy) added*
*Next step: `/gsd-discuss-phase 1` (or `/gsd-plan-phase 1`)*

---

## Phase 5 — API Security Proxy
...
Plans:
- [ ] v9.0-phase-05-01-PLAN.md — Drop static export, add security headers, create server-side Convex client + rate limiter
- [ ] v9.0-phase-05-02-PLAN.md — Create /api/orders and /api/recommendations proxy routes; migrate CheckoutPage + BrewingStudio
...
3. Create `app/api/_lib/convex-server.ts` — server-side `ConvexHttpClient` singleton
4. Create `app/api/_lib/rate-limit.ts` — per-IP sliding-window rate limiter
5. Create `app/api/orders/route.ts` — validates order payload, rate-limits at 20 req/min per IP, proxies to `submitOrder`
6. Create `app/api/recommendations/route.ts` — rate-limits at 10 req/min per IP, proxies to `brewingRecipe` or `flavoredDrink`
...
Nothing — can be executed before or in parallel with v9.0 Phases 1–4 (disjoint files).
